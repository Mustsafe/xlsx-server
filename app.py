...
def build_alias_map(templates: List[str]) -> dict:
    ...
    # JSA / LOTO 강제 매핑
    for tpl in templates:
        key = tpl.lower().replace("_","").replace(" ","")
...
def index():
    return "📰 endpoints: /health, /daily_news, /render_news, /create_xlsx?template=..., /list_templates", 200
...
def create_xlsx():
    ...
    logger.info(f"/create_xlsx?template={raw}")
    ...
    try:
        ...
        logger.warning(f"'{raw}' 미등록 → GPT로 fallback")
        system = {
            "role":"system",
            "content":(
                "당신은 산업안전 문서 템플릿 전문가입니다.\n"
                "아래 컬럼에 맞춰 5개 이상의 항목을 가진 순수 JSON 배열만 출력하세요.\n"
        ...
        try:
            out_df = pd.DataFrame(json.loads(text))
        except:
...
def list_templates():
    ...
    return jsonify({
        "template_list": temps,
        "alias_keys": sorted(build_alias_map(temps).keys())
    })

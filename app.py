...
# ── 텍스트 정규화: 소문자+한글+숫자만 남김 ─────────────────
def sanitize(text: str) -> str:
    return re.sub(r"[^0-9a-z가-힣]", "", text.lower())


# ── alias_map 구축: tpl 전체명 + sanitize 버전 + 접미사 제거 버전 + 강제 키 ──
def build_alias_map(templates: List[str]) -> dict:
    alias = {}
    SUFFIXES = ["점검표","계획서","서식","표","양식"]
    for tpl in templates:
        # 1) 원본 전체명 (소문자/대문자 구분 없이)
        alias[tpl.lower()] = tpl
        # 2) sanitize 전체명
        norm = sanitize(tpl)
        alias[norm] = tpl
        # 3) sanitize에서 주요 접미사 제거
        for suf in SUFFIXES:
            if norm.endswith(suf):
                alias[norm[: -len(suf)]] = tpl
        # 4) 괄호·하이픈·언더바 제거 버전
        cleaned = re.sub(r"[\(\)\-\_]", "", tpl).lower()
        alias[cleaned] = tpl
...
# ── 키워드 → 정식 템플릿명 (100% 매칭 보장) ────────────────
def resolve_keyword(raw: str, templates: List[str], alias_map: dict) -> str:
    # 1) 전처리: “양식/서식/점검표/계획서/표 + (을|를)? + (주세요|줘|달라)? 제거
    tmp = re.sub(
        r"(?:양식|서식|점검표|계획서|표)(?:을|를)?\s*(?:주세요|줘|달라)?$",
        "",
        raw.strip(),
        flags=re.IGNORECASE
    )
    ...
    # 3) prefix 매칭 (sanitize(tpl) 이 key 로 시작하면)
    prefix = [t for t in templates if sanitize(t).startswith(key)]
    if len(prefix) == 1:
        return prefix[0]

    # 4) substring 매칭
    sub = [t for t in templates if key in sanitize(t)]
    if len(sub) == 1:
        return sub[0]
...
def create_xlsx():
    ...
    try:
        ...
        #  existence 체크에 걸리지 않는다면 GPT fallback
        system = {
            "role": "system",
            "content": (
        ...
        user = {
            "role": "user",
            "content": f"템플릿명 '{raw}'의 양식을 JSON 배열로 주세요."
        }
    ...
    for i, col in enumerate(ws.columns, 1):
        width = min(max(len(str(c.value)) for c in col) + 2, 60)

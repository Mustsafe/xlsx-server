...
def explode_json_rows(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for _, row in df.iterrows():
        ...
        text = raw_text.lstrip("\ufeff").strip().replace("“", '"').replace("”", '"')
        arr = None
        if text.startswith("["):
            try:
                arr = json.loads(text)
...
def create_xlsx():
    try:
        ...
        cols=["작업 항목","작성 양식","실무 예시 1","실무 예시 2"]
        if "출처" in out_df.columns:
            cols.append("출처")
        out_df = out_df[cols]
        ...
        filename=f"{tpl}.xlsx"; disp=quote(filename)
        return Response(buf.read(),headers={"Content-Type":"application/vnd.openxmlformats-officedocument.spreadsheetml.sheet","Content-Disposition":f"attachment; filename*=UTF-8''{disp}","Cache-Control":"public, max-age=3600"})
...
def render_news():
    ...
    for n in news:
        ...
        if dt>=cutoff:
            n["날짜"]=dt.strftime("%Y.%m.%d"); filtered.append(n)

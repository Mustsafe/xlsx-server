...
def resolve_keyword(raw: str, templates: List[str], alias_map: dict, freq: dict) -> str:
    ...
    matches = difflib.get_close_matches(key, norms, n=1, cutoff=0.6)
    if matches:
...
def create_xlsx():
    try:
        ...
        try:
            ...
            system = {"role":"system","content":"산업안전 템플릿 전문가입니다. JSON 배열을 5개 이상 생성하세요."}
        ...
        for idx, row in out_df.iterrows():
            ...
            if isinstance(base, str) and len(base.splitlines()) < 3:
                ...
                try:
                    r = openai.chat.completions.create(model="gpt-4o-mini", messages=[sys_msg, usr_msg], max_tokens=300, temperature=0.7)
            ...
            for ex in ["실무 예시 1","실무 예시 2"]:
                ex_base = row.get(ex, "")
                if ex_base:
                    sysg = {"role":"system","content":"구체적 현장 사례를 한 문장으로 설명하세요."}
